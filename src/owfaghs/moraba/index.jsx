...
import calculateTypes from './calculateTypes';

function Moraba(){

    const [selectedType,setSelectedType] = useState(0);
    const [selectedCalculatedType,setSelectedCalculatedType] = useState(0);
    ...
    useEffect(()=>{
        ...
        const outputTable = calculateTypes[selectedCalculatedType].method(types[selectedType].table,abjad);
        if(outputTable){
            setErrorMessage(null);
            setTableValues(outputTable);
        }
    },[abjad,selectedType,selectedCalculatedType]);
    ...
    return (<div className=''>
        <div className='flex gap-4 mx-auto  my-3'>
            ...
            <div className="w-1/3 h-12">
                <select 
                    defaultValue={selectedCalculatedType}
                    value={selectedCalculatedType}
                    onChange={(e)=>setSelectedCalculatedType(e.target.value)}
                    className='w-full h-full px-4'
                >
                    {calculateTypes.map((type,index)=>{
                        return (<option 
                            value={index}
                            key={index}
                        >
